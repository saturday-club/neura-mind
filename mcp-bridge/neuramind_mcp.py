"""MCP server bridge for neuramind's HTTP API.

Exposes neuramind screen context search, summaries, activity, and health
...
logger = logging.getLogger("neuramind_mcp")
logging.basicConfig(stream=sys.stderr, level=logging.INFO)

mcp = FastMCP("neuramind", instructions="Screen context from neuramind")
...
def _get(path: str, params: dict[str, Any] | None = None) -> dict[str, Any]:
    """Make a GET request to neuramind.

    Parameters
    ----------
    path : str
        URL path relative to NEURAMIND_BASE_URL (e.g. "/v1/summaries").
    ...
    try:
        ...
        raise ConnectionError(
            f"neuramind unreachable at {NEURAMIND_BASE_URL}: {exc}"
        ) from exc


def _post(path: str, body: dict[str, Any]) -> dict[str, Any]:
    """Make a POST request to neuramind.

    Parameters
    ----------
    path : str
        URL path relative to NEURAMIND_BASE_URL.
    body : dict
        JSON request body.
...
def search_screen_context(
    ...
    """Search screen captures and OCR text from neuramind.
...
def get_summaries(minutes: int = 60, limit: int = 50) -> str:
    """Get recent activity summaries from neuramind.
...
def get_activity(
    ...
    """Get raw activity data from neuramind.
...
def get_screen_health() -> str:
    """Check if neuramind is running and healthy.
