...
struct NeuraMindFocusState: Codable, Sendable {
    let id: String
    let task: String
    let taskSlug: String?
...
struct NeuraMindFocusBlock: Codable, Sendable {
    let id: String
    let task: String
    let taskSlug: String?
    let startedAt: String
...
enum FocusStateStore {
    ...
    static func loadCurrent() -> NeuraMindFocusState? {
        guard let data = try? Data(contentsOf: currentStatePath) else { return nil }
        return try? JSONDecoder().decode(NeuraMindFocusState.self, from: data)
    }

    static func loadBlocks(limit: Int = 20, includeOpen: Bool = false) -> [NeuraMindFocusBlock] {
        var blocks: [NeuraMindFocusBlock] = readLoggedBlocks(limit: limit)
    ...
    private static func readLoggedBlocks(limit: Int) -> [NeuraMindFocusBlock] {
        ...
        return Array(lines.compactMap { line in
            ...
            return try? JSONDecoder().decode(NeuraMindFocusBlock.self, from: data)
        }.reversed())
    }

    private static func computeDrift(
        current: NeuraMindFocusState?,
