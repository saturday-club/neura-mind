...
5. **All data stays local** — SQLite database in `~/Library/Application Support/NeuraMind/`.
...
├── NeuraMind/                    # All Swift source code
│   ├── App/                     # Application lifecycle and service wiring
│   │   ├── NeuraMindApp.swift    # @main SwiftUI App entry point
...
The app uses `@main struct NeuraMindApp: App` with a `MenuBarExtra` scene (system
...
On init of `NeuraMindApp`, register a global hotkey (Cmd+Shift+Space, keyCode 49)
...
- Path: `~/Library/Application Support/NeuraMind/contextd.sqlite`
...
`~/Library/Application Support/NeuraMind/api_key`.
...
with service `com.neuramind.app`, but it is not currently used for API key storage.
...
Subsystem: `com.neuramind.app`. Levels: debug, info, notice, warning, error.
...
.build/NeuraMind.app/
├── Contents/
│   ├── MacOS/NeuraMind     (copied from .build/debug/NeuraMind)
│   ├── Resources/
...
| Key                                       | Value                                |
|-------------------------------------------|--------------------------------------|
| CFBundleIdentifier                        | com.neuramind.app                   |
| CFBundleExecutable                        | NeuraMind                            |
| CFBundleShortVersionString                | 0.1.0                                |
| LSMinimumSystemVersion                    | 14.0                                 |
| LSUIElement                               | true (no Dock icon)                  |
| NSScreenCaptureUsageDescription           | NeuraMind captures screenshots to...  |
...
dependencies:
  ...
  - hummingbird 2.0+
targets:
  - executableTarget "NeuraMind" (path: "NeuraMind", excludes: "Assets.xcassets")
  - testTarget "NeuraMindTests" (path: "Tests")
```
...
2. **API key in plaintext** — Stored at `~/Library/Application Support/NeuraMind/api_key`.
